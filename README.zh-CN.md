...
| Splat 编辑 | `Picker`、`Brush`、漂浮球自动分析、无人机地图 BBox 清理与背面浮点清理、多步撤销 / 重做、可见 splats `.ply` 保存 |
...
4. 按 `P` 预览整条路径，微调运镜效果。
5. 按 `E` 进入编辑模式，用 `Picker`、`Brush`、`分析漂浮球`、`分析 BBox` 或 `分析背面` 清理噪点。
...
- [x] 增加无人机地图 BBox 清理，用于裁掉二维地图边界外的 splats
- [x] 增加无人机地图背面浮点清理
