...
针对无人机地图场景，`分析 BBox` 会先估计地图自身的二维主方向，再在有向 `U-V` 平面里计算稳健分位数边界。只要 splat 的中心或尺度感知范围超出稳健 U 边界，或超出稳健 V 边界，就会被选中。因此即使大球中心仍在边界附近，只要可见范围伸出地图外侧，也能被 BBox 捕捉。`应用 BBox 过滤` 会隐藏这些候选，并且仍可撤销。

`分析背面` 会复用 BBox 的方向，并且只在这个二维地图边界内部工作。它会把地图划分为 `U-V` grid，在每个 cell 内用 `Y` 分位数估计局部表面高度，然后自动比较低 Y 侧和高 Y 侧，选择离群延伸更强的一侧作为背面。沿这个方向超过局部表面阈值的 splats 会全部被选中，判断时会把 splat 中心和尺度范围一起计入。样本稀疏的 cell 会兜底使用附近 cell 的表面或全局表面。`调试表面` 用于预览局部表面参考点，`调试背面` 用于预览阈值筛出的背面集合，`应用背面过滤` 会隐藏这些候选。
...
- 去掉漂浮噪点
- 预览并应用大型漂浮球自动过滤
- 针对无人机地图场景预览并应用 BBox 清理
- 针对无人机地图场景预览并应用背面浮点清理
- 裁掉场景边缘杂点
- 保存一个更干净的展示版本
